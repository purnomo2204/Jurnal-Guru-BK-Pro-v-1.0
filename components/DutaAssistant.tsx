      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-80 sm:w-96 h-[500px] bg-white border border-slate-200 rounded-3xl shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="p-4 bg-gradient-to-r from-blue-600 to-indigo-600 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-slate-800 font-black text-sm uppercase tracking-tight">Duta Assistant</h3>
                  <p className="text-blue-100 text-[10px] font-bold uppercase tracking-widest">Online</p>
                </div>
              </div>
              <button onClick={() => setIsOpen(false)} className="text-slate-800/70 hover:text-slate-800 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      >
        ...
        <div className="relative w-16 h-16 rounded-full bg-white border-2 border-blue-500 overflow-hidden shadow-xl flex items-center justify-center">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-blue-600/80 py-0.5">
            <p className="text-[8px] font-black text-slate-800 text-center uppercase tracking-tighter">Duta</p>
          </div>
        </div>
        {!isOpen && (
