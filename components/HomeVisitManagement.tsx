...
const HomeVisitManagement: React.FC<HomeVisitManagementProps> = ({ setView, onOpenSettings, students, visits, onAdd, onUpdate, onDelete, teacherData }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 animate-fade-in text-left">
      ...
      <div className="glass-card p-6 rounded-3xl border border-slate-200 mx-4 backdrop-blur-2xl shadow-3xl">
        <h3 className="text-lg font-black text-slate-800 uppercase mb-6 flex items-center gap-3" style={{ fontFamily: 'Orbitron, sans-serif' }}>
          <Home className="w-5 h-5 text-blue-500" /> {editingVisitId ? 'Edit' : 'Dokumentasi'} <span className="text-blue-500 italic">Kunjungan Rumah</span>
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5 md:col-span-2 lg:col-span-3">
            ...
            <div className="w-full h-[250px] rounded-2xl overflow-hidden border border-slate-200 bg-slate-50 relative group shadow-inner">
              {newVisit.address ? (
                <iframe
                  width="100%"
                  height="100%"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="p-3 text-[8px] font-black uppercase tracking-widest text-slate-500">Status</th>
                ...
                filteredVisits.map((visit, idx) => {
                  ...
                  return (
                    <tr key={visit.id} className="hover:bg-white/5 transition-colors group">
                      ...
                      <td className="p-3 text-[10px] font-bold text-slate-800">
                        <span className={`px-2 py-1 rounded-full text-[8px] ${
                          visit.status === 'Selesai' ? 'bg-emerald-100 text-emerald-700' :
                          visit.status === 'Dibatalkan' ? 'bg-rose-100 text-rose-700' :
