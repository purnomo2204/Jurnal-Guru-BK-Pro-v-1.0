...
import { ArrowLeft, User, Hash, Plus, X, FileDown, CalendarDays, Trash2, Edit, FileText, Sparkles, Loader2, Sheet, Download } from 'lucide-react';
                                ...
                                <table className="data-table w-full text-left border-collapse border border-black text-base">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td className="border border-black p-2 text-center text-blue-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Sakit').length}</td>
                                            <td className="border border-black p-2 text-center text-amber-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Ijin').length}</td>
                                            <td className="border border-black p-2 text-center text-rose-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Alpa').length}</td>
                                            <td className="border border-black p-2 text-center text-emerald-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Dispensasi').length}</td>
                                        </tr>
                                        <tr>
                                            <td className="border border-black p-2 text-center font-bold">Genap (2)</td>
                                            <td className="border border-black p-2 text-center text-blue-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Sakit').length}</td>
                                            <td className="border border-black p-2 text-center text-amber-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Ijin').length}</td>
                                            <td className="border border-black p-2 text-center text-rose-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Alpa').length}</td>
                                            <td className="border border-black p-2 text-center text-emerald-600 font-bold">{attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Dispensasi').length}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </section>
                ...
                <div className="lg:col-span-8 space-y-12 text-left">
                  <section className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-y-6 text-xs font-bold px-4">
                      ...
                      <div className="space-y-1"><p className="text-slate-500 text-[9px] uppercase font-black">No. WA / HP</p><p className="text-slate-800 text-[10px]">{student.phone || '-'}</p></div>
                    ...
                    <div className="grid grid-cols-2 gap-y-8 px-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1 font-bold text-[11px] text-slate-800">
                          ...
                          <p className="text-blue-500 font-black">{student.fatherPhone || '-'}</p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1 font-bold text-[11px] text-slate-800">
                          ...
                          <p className="text-pink-500 font-black">{student.motherPhone || '-'}</p>
                        </div>
                      </div>
                      ...
                      <div className="col-span-2 space-y-1">
                        ...
                        <p className="text-slate-800 leading-relaxed italic text-[11px]">{student.parentPhoneWA || '-'}</p>
                      </div>
                    </div>
                  </section>
                </div>
              ...
              <div className="lg:col-span-1 glass-card p-6 rounded-[2.5rem] border border-slate-200 shadow-lg bg-white/90">
                ...
                <div className="space-y-3">
                  {eventLogs.length === 0 ? (
                    ...
                    eventLogs.map(log => (
                      <div key={log.id} className="p-1.5 bg-white/60 rounded-xl border border-slate-200">
                        <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">{log.date} - {log.time}</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px] font-bold">
                      <div className="text-blue-600">S: {attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Sakit').length}</div>
                      <div className="text-amber-600">I: {attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Ijin').length}</div>
                      <div className="text-rose-600">A: {attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Alpa').length}</div>
                      <div className="text-emerald-600">D: {attendanceRecords.filter(r => r.semester === 'Ganjil' && r.status === 'Dispensasi').length}</div>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-200 pb-1">Semester Genap</p>
                    <div className="grid grid-cols-2 gap-2 text-[10px] font-bold">
                      <div className="text-blue-600">S: {attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Sakit').length}</div>
                      <div className="text-amber-600">I: {attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Ijin').length}</div>
                      <div className="text-rose-600">A: {attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Alpa').length}</div>
                      <div className="text-emerald-600">D: {attendanceRecords.filter(r => r.semester === 'Genap' && r.status === 'Dispensasi').length}</div>
                    </div>
                  </div>
                </div>
