            ...
            rows: [
              new TableRow({
                children: [
                  ...
                  new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "NISN / NIS", bold: true })], alignment: AlignmentType.CENTER })], width: { size: 20, type: WidthType.PERCENTAGE } }),
                  new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "NAMA SISWA", bold: true })], alignment: AlignmentType.CENTER })], width: { size: 45, type: WidthType.PERCENTAGE } }),
            ...
            <table className="w-full text-left">
               <thead>
                  <tr className="bg-slate-100/50 text-blue-400 text-[8px] font-black uppercase tracking-widest border-b border-slate-200">
                     ...
                     <th className="p-2 w-24 cursor-pointer hover:bg-slate-200/50 transition-colors" onClick={() => requestSort('nisn')}>
                       NISN / NIS {sortConfig?.key === 'nisn' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                     </th>
                     <th className="p-2 cursor-pointer hover:bg-slate-200/50 transition-colors" onClick={() => requestSort('name')}>
                       Nama Siswa {sortConfig?.key === 'name' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                     </th>
                     <th className="p-2 w-10 text-center cursor-pointer hover:bg-slate-200/50 transition-colors" onClick={() => requestSort('gender')}>
                       L/P {sortConfig?.key === 'gender' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                     </th>
                     <th className="p-2 cursor-pointer hover:bg-slate-200/50 transition-colors" onClick={() => requestSort('address')}>
               ...
               <tbody className="divide-y divide-white/5">
                  {filteredStudents.length === 0 ? (
                    <tr><td colSpan={5} className="p-8 text-center text-slate-500 font-bold uppercase tracking-widest text-[8px] italic">Tidak ada data siswa.</td></tr>
                  ) : filteredStudents.map((s, idx) => (
                    <tr key={s.id} className="hover:bg-white/5 transition-colors group">
                       <td className="p-2 text-center text-[9px] font-bold text-slate-500">{idx + 1}</td>
                       <td className="p-2">
                          <div className="text-[9px] font-black text-slate-800">{s.nisn || '-'}</div>
                          <div className="text-[8px] text-blue-400 font-black mt-0.5">{s.nis || '-'}</div>
                       </td>
                       <td className="p-2">
