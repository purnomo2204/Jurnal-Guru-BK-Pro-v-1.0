...
import { ViewMode, Student, CounselingLog, EventLog, Violation, Achievement, TeacherData } from '../types';
...
const AutomatedReports: React.FC<AutomatedReportsProps> = ({ 
  ...
  const handleGenerateWeeklySummary = async () => {
    ...
    const reportData = {
      type: 'weekly',
      title: `Ringkasan Aktivitas Mingguan: ${selectedWeek}`,
      week: selectedWeek,
      stats: {
        totalCounseling: counselingLogs.length,
    ...
    setGeneratedReport(reportData);
    setIsGenerating(false);
    showNotification('Ringkasan berhasil disusun!', 'success');
  };
          ...
          ...(generatedReport.type === 'student' ? [
            ...
            new Paragraph({
              children: [
                ...
                new TextRun({ text: generatedReport.week }),
              ],
            }),
            new Paragraph({
              children: [
            ...
            new Paragraph({ text: `Total Layanan Konseling: ${generatedReport.stats.totalCounseling}` }),
            new Paragraph({ text: `Total Catatan Anekdot: ${generatedReport.stats.totalEvents}` }),
            new Paragraph({ text: `Total Pelanggaran: ${generatedReport.stats.totalViolations}` }),
            new Paragraph({ text: `Total Prestasi: ${generatedReport.stats.totalAchievements}` }),
          ]),
          ...
          <div className="glass-card p-4 rounded-2xl border border-primary/10 bg-white shadow-xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
              {generatedReport.type === 'student' ? (
                ...
                <>
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
                    <p className="text-[6px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Total Konseling</p>
                    <p className="text-lg font-black text-slate-900">{generatedReport.stats.totalCounseling}</p>
                  </div>
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
                    <p className="text-[6px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Total Anekdot</p>
                    <p className="text-lg font-black text-slate-900">{generatedReport.stats.totalEvents}</p>
                  </div>
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
                    <p className="text-[6px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Total Pelanggaran</p>
                    <p className="text-lg font-black text-rose-600">{generatedReport.stats.totalViolations}</p>
                  </div>
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
                    <p className="text-[6px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Total Prestasi</p>
            ...
            <div className="p-4 bg-white rounded-2xl border border-slate-200 shadow-inner">
              ...
              <div className="text-xs text-slate-600 leading-relaxed font-medium">
                {generatedReport.type === 'student' ? (
                  ...
                  <p>
                    Rekapitulasi aktivitas bimbingan dan konseling untuk minggu <span className="text-slate-800 font-bold">{generatedReport.week}</span> menunjukkan total {generatedReport.stats.totalCounseling} layanan yang telah diberikan kepada siswa. 
