    ...
    return (
      <div className="max-w-2xl mx-auto glass-card p-3 rounded-xl shadow-lg animate-fade-in text-left mb-4 border border-white">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 gap-2">
          <h2 className="text-lg font-black text-slate-900 uppercase tracking-tighter" style={{ fontFamily: 'Orbitron, sans-serif' }}>
            Input <span className="text-blue-600 italic">Layanan BK</span>
          </h2>
          <div className="bg-blue-50/50 px-1.5 py-0.5 rounded-lg border border-blue-100 shadow-sm backdrop-blur-xl">
            <label className="text-[6px] font-black text-blue-600 uppercase block mb-0 tracking-widest">Tahun Ajaran</label>
            <input type="text" value={formData.academicYear} onChange={e => setFormData({...formData, academicYear: e.target.value})} className="bg-transparent border-none p-0 text-slate-800 font-black focus:ring-0 text-[8px] w-14" />
          </div>
        </div>
        ...
        <form id="counseling-form" onSubmit={handleSubmit} className="space-y-2.5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            <div className="space-y-0.5">
              <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><Calendar className="w-2 h-2 text-blue-500" /> Tanggal Kejadian</label>
              <input type="date" required value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none" />
            </div>
            <div className="grid grid-cols-2 gap-1.5">
              <div className="space-y-0.5">
                <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><Clock className="w-2 h-2 text-blue-500" /> Mulai</label>
                <input type="time" required value={formData.startTime} onChange={e => setFormData({...formData, startTime: e.target.value})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none" />
              </div>
              <div className="space-y-0.5">
                <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><Clock className="w-2 h-2 text-blue-500" /> Selesai</label>
                <input type="time" required value={formData.endTime} onChange={e => setFormData({...formData, endTime: e.target.value})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            <div className="space-y-0.5">
              <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em]">Komponen Layanan</label>
              <select value={formData.component} onChange={e => handleComponentChange(e.target.value as ServiceComponent)} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none appearance-none">
            ...
            <div className="space-y-0.5">
              <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em]">Strategi Layanan</label>
              <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value as CounselingType, studentId: '', groupId: '', absentStudentIds: []})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none appearance-none">
          ...
          <div className="space-y-0.5">
            <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em]">Bidang Bimbingan</label>
            <select value={formData.aspect} onChange={e => setFormData({...formData, aspect: e.target.value as CounselingAspect})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none appearance-none">
          ...
          {formData.component !== 'Dukungan Sistem' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100 shadow-inner">
              {isCurrentKelompok ? (
                <div className="space-y-0.5 md:col-span-2">
                  <label className="text-[7px] font-black text-blue-600 uppercase tracking-[0.2em] flex items-center gap-1"><Layers className="w-2 h-2" /> Pilih Kelompok Bimbingan</label>
                  <select required value={formData.groupId} onChange={e => setFormData({...formData, groupId: e.target.value, absentStudentIds: []})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none appearance-none">
                ...
                <>
                  <div className="space-y-0.5">
                    <label className="text-[7px] font-black text-blue-600 uppercase tracking-[0.2em] flex items-center gap-1"><Filter className="w-2 h-2" /> 1. Filter Kelas</label>
                    <select value={inputClassFilter} onChange={e => { setInputClassFilter(e.target.value); setFormData({...formData, studentId: ''}); setSelectedClasses([]); }} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none appearance-none">
                  ...
                  <div className="space-y-0.5">
                    {isCurrentKlasikal && inputClassFilter === '' ? (
                      <>
                        <label className="text-[7px] font-black text-blue-600 uppercase tracking-[0.2em] flex items-center gap-1"><Users className="w-2 h-2" /> 2. Pilih Kelas (Bisa Lebih Dari Satu)</label>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-1 p-1 h-20 overflow-y-auto custom-scrollbar bg-white rounded-lg border border-slate-100 shadow-inner">
                          {availableClasses.map(c => (
                            <div key={c} onClick={() => toggleSelectedClass(c)} className={`p-1 rounded-md border cursor-pointer transition-all flex items-center gap-1 ${selectedClasses.includes(c) ? 'bg-blue-50 border-blue-200 text-blue-700 shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-blue-200'}`}>
                              <span className="text-[7px] font-bold uppercase tracking-tight truncate">Kelas {c}</span>
                            </div>
                          ))}
                        </div>
                      </>
                      ...
                      <>
                        <label className="text-[7px] font-black text-blue-600 uppercase tracking-[0.2em] flex items-center gap-1"><Users className="w-2 h-2" /> 2. Peserta Didik</label>
                        <div className="w-full p-1.5 text-[9px] input-cyber rounded-lg bg-blue-50 text-blue-700 font-black flex items-center justify-center border border-blue-200">
                          SELURUH SISWA KELAS {inputClassFilter}
                        </div>
                      </>
                    ) : (
                      <>
                        <label className="text-[7px] font-black text-blue-600 uppercase tracking-[0.2em] flex items-center gap-1"><Users className="w-2 h-2" /> 2. Pilih Peserta Didik</label>
                        <select required={!isCurrentKlasikal} value={formData.studentId} onChange={e => setFormData({...formData, studentId: e.target.value})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none appearance-none">
          ...
          {(isCurrentKlasikal || isCurrentKelompok) && (
            <div className="space-y-2.5 p-2 bg-slate-50 rounded-lg border border-slate-100 shadow-inner">
               <div className="space-y-0.5">
                  <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><FileText className="w-2 h-2 text-blue-500" /> Topik Bimbingan</label>
                  <input required value={formData.topic || ''} onChange={e => setFormData({...formData, topic: e.target.value})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none" placeholder="Judul materi bimbingan..." />
               </div>
               
               {isCurrentKlasikal && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                    <a href="https://gemini.google.com/share/5c45a8ccd89f" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1 p-1.5 rounded-lg bg-blue-600 text-white border border-blue-700 hover:bg-blue-700 transition-all font-black text-[7px] uppercase tracking-widest shadow-md">
                      <ExternalLink className="w-2 h-2" /> RPLBK (Versi ARKA)
                    </a>
                    <a href="https://gemini.google.com/share/8a632c3a54a9" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1 p-1.5 rounded-lg bg-purple-600 text-white border border-purple-700 hover:bg-purple-700 transition-all font-black text-[7px] uppercase tracking-widest shadow-md">
                      <ExternalLink className="w-2 h-2" /> RPLBK (Deep Learning)
                    </a>
                  </div>
               )}
               ...
               {isSpecialField(formData.type || '') && (
                 <div className="space-y-0.5">
                    <label className="text-[7px] font-black text-blue-600 uppercase tracking-[0.2em] flex items-center gap-1"><Target className="w-2 h-2" /> TUJUAN BIMBINGAN</label>
                    <textarea required value={formData.purpose || ''} onChange={e => setFormData({...formData, purpose: e.target.value})} className="w-full p-1.5 text-[9px] h-14 leading-relaxed input-cyber rounded-lg outline-none resize-none" placeholder="Tujuan yang ingin dicapai..." />
                 </div>
               )}
               ...
               {formData.type === 'Bimbingan Kelompok' && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-2 animate-in fade-in slide-in-from-top-1 duration-300">
                   <a 
                     ...
                     target="_blank" 
                     rel="noreferrer" 
                     className="flex items-center justify-center gap-2 p-2 rounded-lg bg-blue-600 text-white border border-blue-700 hover:bg-blue-700 transition-all font-black text-[7px] md:text-[8px] uppercase tracking-[0.1em] md:tracking-[0.2em] shadow-md group"
                   >
                   ...
                   <a 
                     ...
                     target="_blank" 
                     rel="noreferrer" 
                     className="flex items-center justify-center gap-2 p-2 rounded-lg bg-indigo-600 text-white border border-indigo-700 hover:bg-indigo-700 transition-all font-black text-[7px] md:text-[8px] uppercase tracking-[0.1em] md:tracking-[0.2em] shadow-md group"
                   >
                     <ExternalLink className="w-3 h-3 group-hover:scale-110 transition-transform" /> 
               ...
               <div className="space-y-1">
                  <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><UserMinus className="w-2 h-2 text-rose-500" /> Siswa Tidak Hadir</label>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1 p-1 h-20 overflow-y-auto custom-scrollbar bg-white rounded-lg border border-slate-100 shadow-inner">
                      {(isCurrentKlasikal ? filteredStudentsForInput : students.filter(s => groups.find(g => g.id === formData.groupId)?.studentIds.includes(s.id))).map(s => (
                        <div key={s.id} onClick={() => toggleAbsentStudent(s.id)} className={`p-1 rounded-md border cursor-pointer transition-all flex items-center gap-1 ${formData.absentStudentIds?.includes(s.id) ? 'bg-rose-50 border-rose-200 text-rose-700 shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-blue-200'}`}>
                          <span className="text-[7px] font-bold uppercase tracking-tight truncate">{s.name}</span>
                        </div>
                      ))}
                  </div>
               </div>
          ...
          {isConsultation(formData.type || '') && (
            <div className="space-y-0.5">
              <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><UserPlus className="w-2 h-2 text-emerald-500" /> Nama Pihak Terkait</label>
              <input required value={formData.consultantName || ''} onChange={e => setFormData({...formData, consultantName: e.target.value})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none font-arial" placeholder="Nama Guru / Ortu / Instansi..." />
            </div>
          )}

          <div className="space-y-0.5">
            <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><BookOpen className="w-2 h-2 text-blue-500" /> Uraian Kegiatan</label>
            <textarea required value={formData.result} onChange={e => setFormData({...formData, result: e.target.value})} placeholder="Temuan selama bimbingan..." className="w-full p-1.5 h-16 text-[9px] leading-relaxed input-cyber rounded-lg outline-none resize-none" />
          </div>
          ...
          {formData.type === 'Konseling Individu' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 animate-in fade-in slide-in-from-top-1 duration-300">
              <a 
                ...
                target="_blank" 
                rel="noreferrer" 
                className="flex items-center justify-center gap-2 p-2 rounded-lg bg-emerald-600 text-white border border-emerald-700 hover:bg-emerald-700 transition-all font-black text-[7px] md:text-[8px] uppercase tracking-[0.1em] md:tracking-[0.2em] shadow-md group"
              >
                <ExternalLink className="w-3 h-3 group-hover:scale-110 transition-transform" /> 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            <div className="space-y-0.5">
              <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em]">Hasil / Kondisi Akhir</label>
              <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as CounselingStatus})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none appearance-none">
            ...
            <div className="space-y-0.5">
              <div className="flex justify-between items-center mb-0">
                <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em]">Tindak Lanjut</label>
                <button type="button" onClick={handleAISuggestFollowUp} className="flex items-center gap-1 text-blue-600 font-black uppercase tracking-widest text-[7px] hover:text-blue-500 transition-colors"><Sparkles className="w-1.5 h-1.5" /> Smart AI</button>
              </div>
              <input required value={formData.followUp} onChange={e => setFormData({...formData, followUp: e.target.value})} className="w-full p-1.5 text-[9px] input-cyber rounded-lg outline-none" />
            </div>
          </div>

          <div className="space-y-0.5">
            <label className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1"><Info className="w-2 h-2 text-slate-500" /> Catatan / Keterangan (Opsional)</label>
            <textarea value={formData.notes || ''} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full p-1.5 h-10 text-[9px] input-cyber rounded-lg outline-none resize-none" placeholder="Tambahkan keterangan atau catatan singkat..." />
          </div>
            ...
            <thead>
              <tr className="bg-slate-50 text-blue-600 text-[8px] font-black uppercase tracking-widest border-b border-slate-100">
                ...
                {isSpecialField(filterType) ? (
                  <th className="p-3">Topik & Tujuan Bimbingan</th>
                ) : (
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center px-4" onClick={() => setPreviewLog(null)}>
          ...
          <div className="relative glass-card w-full max-w-md rounded-2xl border border-white p-5 shadow-2xl bg-white" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2.5 max-h-[60vh] overflow-y-auto pr-1.5 custom-scrollbar">
               ...
               {previewLog.topic && <div className="p-2.5 bg-indigo-50 rounded-lg border border-indigo-100"><label className="text-[7px] font-black text-indigo-400 uppercase tracking-widest block mb-0">Topik / Materi</label><p className="text-[10px] font-black text-slate-800 italic uppercase">"{previewLog.topic}"</p></div>}
