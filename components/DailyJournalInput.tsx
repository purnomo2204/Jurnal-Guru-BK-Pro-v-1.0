...
const DailyJournalInput: React.FC<{ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onAdd({ ...formData, id: Date.now().toString() } as DailyJournal);
    clearFormData();
  ...
  return (
    <div className="max-w-4xl mx-auto glass-card p-4 rounded-xl shadow-lg animate-fade-in text-left mb-6 border border-white">
      <h2 className="text-lg font-black text-slate-900 uppercase tracking-tighter mb-4">JURNAL HARIAN BK</h2>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="w-[50%] min-w-[180px] space-y-1">
            <label className="text-[8px] font-black text-slate-500 uppercase tracking-[0.2em]">Jenis Kegiatan</label>
            <select value={formData.activityType} onChange={e => setFormData({...formData, activityType: e.target.value as any})} className="w-full p-2 text-[10px] input-cyber rounded-lg outline-none appearance-none">
          ...
          <div className="w-[30%] min-w-[120px] space-y-1">
            <label className="text-[8px] font-black text-slate-500 uppercase tracking-[0.2em]">Kelas</label>
            <input type="text" value={formData.className} onChange={e => setFormData({...formData, className: e.target.value})} className="w-full p-2 text-[10px] input-cyber rounded-lg outline-none" placeholder="Contoh: X-1" />
          </div>
        </div>
