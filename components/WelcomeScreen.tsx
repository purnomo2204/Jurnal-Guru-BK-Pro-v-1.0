...
import { ArrowRight, HeartHandshake, Lock, Eye, EyeOff, AlertCircle, User, CheckCircle2, Sparkles, BarChart3, BookOpen, ShieldCheck } from 'lucide-react';
import { TeacherData } from '../types';
import { formatAcademicTitle } from '../src/lib/nameFormatter';
import { motion, AnimatePresence } from 'framer-motion';

const WelcomeScreen: React.FC<{ onEnter: () => void; teacherData: TeacherData }> = ({ onEnter, teacherData }) => {
                ...
                <div className="flex flex-col items-center mb-6">
                  {teacherData?.photo ? (
                    <img 
                      ...
                      alt="Foto Guru" 
                      className="w-20 h-20 object-cover rounded-full border-4 border-white shadow-lg mb-3"
                      referrerPolicy="no-referrer"
                    />
                  ) : (
                    <div className="w-20 h-20 bg-blue-50 rounded-full border-4 border-white shadow-lg mb-3 flex items-center justify-center text-blue-400">
                       <User className="w-10 h-10" />
                    </div>
                  )}
                  <h3 className="text-base font-bold text-slate-800">{formatAcademicTitle(teacherData?.name || 'Bapak/Ibu Guru BK')}</h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{teacherData?.school || 'Instansi Pendidikan'}</p>
                </div>
