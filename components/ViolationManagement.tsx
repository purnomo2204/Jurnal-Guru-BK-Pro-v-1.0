...
const ViolationManagement: React.FC<ViolationManagementProps> = ({ 
  ...
  }, [violations, students, searchQuery, classFilter, statusFilter]);
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <input type="date" required value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full input-cyber rounded-lg p-2 text-[10px] outline-none" />
            </div>
            <div className="space-y-1">
          ...
          <div className="space-y-1.5">
            ...
            <input required value={formData.violation} onChange={e => setFormData({...formData, violation: e.target.value})} className="w-full input-cyber rounded-xl p-3 text-[10px] outline-none" placeholder="Contoh: Terlambat, Tidak memakai atribut, dll..." />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea required value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full input-cyber rounded-xl p-3 h-20 text-[10px] leading-relaxed outline-none" placeholder="Ceritakan detail kejadian kasus..." />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea required value={formData.actionTaken} onChange={e => setFormData({...formData, actionTaken: e.target.value})} className="w-full input-cyber rounded-xl p-3 h-16 text-[10px] leading-relaxed outline-none" placeholder="Tindakan atau sanksi yang diberikan..." />
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <select 
              ...
              className="bg-white border border-slate-200 rounded-lg px-3 py-2 text-[10px] font-bold text-slate-700 focus:ring-2 focus:ring-rose-600/10 outline-none transition-all shadow-sm"
            >
              <option value="SEMUA">KELAS</option>
              {availableClasses.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          ...
          <button onClick={() => { resetForm(); setIsInputMode(true); }} className="bg-rose-600/20 text-rose-400 border border-rose-500/30 px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-lg hover:bg-rose-600 hover:text-white">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="p-2 text-[8px] font-black uppercase text-slate-500 tracking-widest">Tingkat</th>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredViolations.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-6 text-center text-[10px] text-slate-500 italic">Belum ada catatan kasus.</td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-white/5 transition-colors group">
                      <td className="p-2">
                        ...
                        <p className="text-[8px] font-black text-rose-500 uppercase">{student?.className || '-'}</p>
                      </td>
                      <td className="p-2 text-[10px] text-slate-500">
                        {new Date(v.date).toLocaleDateString('id-ID', {day:'2-digit', month:'short', year:'numeric'})}
                      </td>
                      <td className="p-2">
                        <p className="text-[10px] font-bold text-slate-700">{v.violation}</p>
                        <p className="text-[9px] text-slate-500 italic line-clamp-1">{v.description}</p>
                      </td>
                      <td className="p-2">
                        <span className={`text-[7px] font-black px-1.5 py-0.5 rounded-full uppercase tracking-widest border ${v.level === 'berat' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : v.level === 'sedang' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'}`}>
                          {v.level}
                        </span>
                      </td>
                      <td className="p-2 text-[9px] text-slate-500 max-w-[120px] truncate">
                        {v.actionTaken}
                      </td>
                      <td className="p-2">
