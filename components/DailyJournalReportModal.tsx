...
const DailyJournalReportModal: React.FC<DailyJournalReportModalProps> = ({ journals, onClose }) => {
  ...
  const [kopSurat, setKopSurat] = useState({
    ...
    alamat: 'Jl. Pendidikan No. 1, Kota Contoh, Kode Pos 12345',
    kontak: '(0123) 456789 | Email: info@sman1contoh.sch.id',
    website: 'Website: www.sman1contoh.sch.id'
  });

  const [tandaTangan, setTandaTangan] = useState({
    tempatTanggal: 'Kota Contoh, 25 Maret 2026',
    jabatanKepala: 'Kepala Sekolah',
    namaKepala: 'Dr. Budi Santoso, M.Pd.',
    nipKepala: 'NIP. 19700101 199512 1 001',
    jabatanGuru: 'Guru Bimbingan dan Konseling',
    namaGuru: 'Purnomo Wiwit, S.Pd.',
    nipGuru: 'NIP. 19850202 201001 1 002'
  });
        ...
        <div className="flex-1 overflow-y-auto bg-slate-100 p-6 custom-scrollbar flex justify-center">
          ...
          {viewMode === 'EDIT' ? (
            ...
            >
              <div 
                ref={reportRef} 
                className="p-10 bg-white text-black font-serif"
                ...
                <table className="w-full border-collapse border border-black text-sm mb-8">
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      <th className="border border-black p-2 text-center w-32">Hari, Tanggal</th>
                      <th className="border border-black p-2 text-center w-24">Waktu</th>
                      <th className="border border-black p-2 text-center w-24">Kelas</th>
                      <th className="border border-black p-2 text-center w-32">Jenis Kegiatan</th>
                      <th className="border border-black p-2 text-center w-48">Nama Kegiatan</th>
                      <th className="border border-black p-2 text-center w-40">Tempat</th>
                      <th className="border border-black p-2 text-center">Deskripsi / Hasil</th>
                      <th className="border border-black p-2 text-center w-32">Keterangan</th>
                    </tr>
                  </thead>
                  <tbody>
                    {journals.length === 0 ? (
                      ...
                      journals.map((journal, index) => (
                        <tr key={journal.id}>
                          ...
                          <td className="border border-black p-2">{journal.day}, {journal.date}</td>
                          <td className="border border-black p-2 text-center">{journal.time}</td>
                          <td className="border border-black p-2 text-center">{journal.className || '-'}</td>
                          <td className="border border-black p-2 text-center">{journal.activityType}</td>
                          <td className="border border-black p-2">{journal.activityName}</td>
                          <td className="border border-black p-2">{journal.place}</td>
                          <td className="border border-black p-2">{journal.description}</td>
                          <td className="border border-black p-2 text-center">{journal.status}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
