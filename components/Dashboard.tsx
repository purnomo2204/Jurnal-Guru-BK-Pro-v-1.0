...
import { ViewMode, Student, CounselingLog, TeacherData, AppearanceConfig, CounselingSchedule, AKPDResponse, AKPDQuestion } from '../types';
import { 
...
import DutaAssistant from './DutaAssistant';
    ...
    <div ref={setNodeRef} style={style} className="h-full">
      ...
      >
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h3 className={`${category === 'KOLABORASI TIM' ? 'text-slate-800' : (category === 'Laporan' ? 'text-indigo-600' : 'text-slate-800')} font-bold text-[11px] uppercase tracking-wider leading-tight`} style={{ fontFamily: 'Orbitron, sans-serif' }}>{item.title}</h3>
            <p className={`${category === 'KOLABORASI TIM' ? 'text-slate-800/70' : 'text-slate-500'} text-[9px] mt-1 line-clamp-1 font-medium`}>{item.desc}</p>
          </div>
        </div>
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const [layout, setLayout] = useState<{ [key: string]: string[] }>(() => {
    const defaultLayout = {
      ...
      'PUSAT PENGATURAN': menuItems['PUSAT PENGATURAN'].map(i => i.title)
    };
    ...
    if (parsed) {
      // Migration: rename Konfigurasi to Pengaturan Aplikasi
      if (parsed['PUSAT PENGATURAN']) {
        parsed['PUSAT PENGATURAN'] = parsed['PUSAT PENGATURAN'].map((t: string) => t === 'Konfigurasi' ? 'Pengaturan Aplikasi' : t);
      }
      if (parsed['Input Data']) {
  ...
  const allMenuItems = useMemo(() => {
    ...
    // Inject actions for PUSAT PENGATURAN
    items['PUSAT PENGATURAN'] = [
      { 
  ...
  return (
    <div className="animate-fade-in transition-colors duration-300">
      ...
      <div className="relative z-10">
        ...
        {category === 'Dashboard' && (
          <>
            {/* Academic Events Reminder */}
            {academicEvents && academicEvents.filter(e => e.reminder && new Date(e.date) >= new Date(new Date().setHours(0,0,0,0))).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).slice(0, 5).length > 0 && (
              <div className="mb-8 p-3 bg-white border border-amber-100 rounded-[2rem] relative overflow-hidden shadow-xl shadow-amber-500/5 animate-fade-in">
                <div className="absolute top-0 right-0 w-64 h-64 bg-amber-50 blur-[80px] -mr-32 -mt-32 rounded-full pointer-events-none" />
                <h2 className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3 flex items-center gap-2 px-2" style={{ fontFamily: 'Orbitron, sans-serif' }}>
                  <Bell className="w-3.5 h-3.5" /> Pengingat Kegiatan Akademik
                </h2>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2 relative z-10">
                  {academicEvents.filter(e => e.reminder && new Date(e.date) >= new Date(new Date().setHours(0,0,0,0))).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).slice(0, 5).map(event => (
                    <div key={event.id} className="bg-amber-50/30 border border-amber-100 p-3 rounded-2xl flex flex-col gap-2 group hover:bg-white hover:border-amber-300 transition-all shadow-sm">
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-1.5 text-amber-600 bg-white border border-amber-100 px-2 py-1 rounded-lg">
                           <Calendar className="w-3 h-3" />
                           <span className="text-[8px] font-black uppercase tracking-wider">{new Date(event.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
                        </div>
                      </div>
                      <div>
                        <h3 className="text-[11px] font-bold text-slate-800 leading-tight truncate">{event.title}</h3>
                        <p className="text-[8px] text-amber-500 uppercase tracking-wider font-bold mt-0.5">
                          {event.type === 'pembelajaran' ? 'Belajar' : event.type === 'libur' ? 'Libur Nasional' : event.type === 'mgbk' ? 'MGBK' : event.type === 'ujian' ? 'Ujian' : event.type === 'libur_sekolah' ? 'Libur Sekolah' : 'Lain-lain'}
                        </p>
                      </div>
                      {event.description && (
                        <div className="text-[9px] text-slate-500 italic line-clamp-1 bg-white p-2 rounded-xl border border-amber-50">
                          {event.description}
                        </div>
                      )}
                    </div>
            ...
            {/* Counseling Schedule Reminder */}
            {upcomingSchedules.length > 0 && (
              <div className="mb-8 p-3 bg-white border border-rose-100 rounded-[2rem] relative overflow-hidden shadow-xl shadow-rose-500/5 animate-fade-in">
                <div className="absolute top-0 right-0 w-64 h-64 bg-rose-50 blur-[80px] -mr-32 -mt-32 rounded-full pointer-events-none" />
                <h2 className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-3 flex items-center gap-2 px-2" style={{ fontFamily: 'Orbitron, sans-serif' }}>
                  <Calendar className="w-3.5 h-3.5" /> Pengingat Jadwal Konseling
                </h2>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 relative z-10">
                  {upcomingSchedules.map(schedule => (
                    <div key={schedule.id} className="bg-rose-50/30 border border-rose-100 p-3 rounded-2xl flex flex-col gap-2 group hover:bg-white hover:border-rose-300 transition-all shadow-sm">
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-1.5 text-rose-600 bg-white border border-rose-100 px-2 py-1 rounded-lg">
                           <Clock className="w-3 h-3" />
                           <span className="text-[8px] font-black uppercase tracking-wider">{new Date(schedule.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })} • {schedule.time}</span>
                        </div>
                        {onCompleteSchedule && (
                          <button 
                            onClick={(e) => {
                            ...
                            className="p-1.5 bg-white hover:bg-emerald-500 text-slate-600 hover:text-white rounded-lg transition-all border border-slate-100 hover:border-emerald-500 group/btn"
                      ...
                      <div>
                        <h3 className="text-[11px] font-bold text-slate-800 leading-tight truncate">{schedule.studentName}</h3>
                        <p className="text-[8px] text-rose-400 uppercase tracking-wider font-bold mt-0.5">{schedule.className}</p>
                      </div>
                      <div className="text-[9px] text-slate-500 italic line-clamp-1 bg-white p-2 rounded-xl border border-rose-50">
                        "{schedule.topic}"
                      </div>
                    </div>
                  ))}
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Main Content */}
            <main className={`${category === 'Dashboard' ? 'hidden' : 'lg:col-span-3'} space-y-8`}>
              {(() => {
                ...
                const filteredCategories = category && category !== 'Dashboard'
                  ? fullOrder.filter(key => key === category)
                  : [];
                
                return filteredCategories.map((categoryKey) => {
                  const itemTitles = layout[categoryKey] || menuItems[categoryKey as keyof typeof menuItems].map(i => i.title);
                  
                  return (
                    <section key={categoryKey}>
                      <h2 className="text-sm font-black text-slate-500 uppercase tracking-widest mb-4 px-1" style={{ fontFamily: 'Orbitron, sans-serif' }}>{categoryKey}</h2>
                      <SortableContext 
                        items={itemTitles} 
                          ...
                          {itemTitles.map((title) => {
                            ...
                            return (
                              <SortableItem 
                                ...
                                isSpecial={categoryKey === 'PUSAT PENGATURAN'}
            ...
            {category === 'Dashboard' && (
              <aside className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {akpdStats.length > 0 && (
                  <section>
                    <h2 className="text-sm font-black text-slate-500 uppercase tracking-widest mb-4 px-1" style={{ fontFamily: 'Orbitron, sans-serif' }}>Statistik AKPD</h2>
                    <div className="p-6 rounded-[2.5rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/20">
                      <div className="h-48">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              ...
                              stroke="none"
                            >
                              {akpdStats.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={entry.color} />
                              ))}
                            </Pie>
                            <Tooltip 
                              contentStyle={{ backgroundColor: '#ffffff', borderColor: '#f1f5f9', borderRadius: '1rem', fontSize: '12px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}
                              itemStyle={{ color: '#1e293b', fontWeight: 'bold' }}
                              formatter={(value: number) => [`${value}%`, 'Persentase "Ya"']}
                            />
                          </PieChart>
                        </ResponsiveContainer>
                      </div>
                      <div className="grid grid-cols-2 gap-3 mt-6">
                        {akpdStats.map((stat, idx) => (
                          <div key={idx} className="flex items-center gap-2.5 bg-slate-50 p-2 rounded-xl border border-slate-100">
                            ...
                            <span className="text-[9px] font-black text-slate-600 uppercase tracking-wider">{stat.name}</span>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      <DutaAssistant studentsCount={students.length} />
