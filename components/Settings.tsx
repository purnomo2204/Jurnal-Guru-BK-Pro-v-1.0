...
import { 
  ...
  Monitor, Apple, CheckCircle, Bell
} from 'lucide-react';
...
const Settings: React.FC<SettingsProps> = ({ 
  ...
  onResetData, onSyncCloud, db, auth
}) => {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'report' | 'subjects' | 'cloud' | 'backup' | 'firebase' | 'appearance' | 'notifications'>(initialTab);
...
function doGet(e) {
  ...
  const verifySchoolPassword = (e: React.FormEvent) => {
    e.preventDefault();
    const currentSchoolPass = teacherData.schoolPassword || '@Spero1951';
  ...
  const handleVerifyOldPass = (e: React.FormEvent) => {
    e.preventDefault();
    const currentPass = changePassMode === 'akses' ? (teacherData.accessPassword || '@Spero123') : (teacherData.schoolPassword || '@Spero1951');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 pb-10 animate-fade-in text-left">
      ...
      <div className="grid grid-cols-1 gap-4 mx-4">
        {activeTab === 'profile' && (
          <div className="glass-card p-5 rounded-[1.5rem] border border-slate-200 space-y-5 shadow-xl animate-fade-in relative overflow-hidden">
             <div className="flex items-center gap-3 border-b border-slate-200 pb-4">
              <div className="w-10 h-10 bg-blue-600/10 rounded-xl flex items-center justify-center text-blue-500 border border-blue-500/20 shadow-lg"><User className="w-5 h-5" /></div>
              <div>
                <h3 className="text-lg font-black text-slate-800 tracking-tight uppercase italic">Identitas Guru & Instansi</h3>
                <p className="label-luxe text-[7px] text-slate-500">Kelola informasi profil profesional dan atribut laporan</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="md:col-span-1 space-y-4">
                <div className="space-y-1.5">
                  ...
                  <div onClick={() => teacherPhotoInputRef.current?.click()} className="relative aspect-square w-full max-w-[120px] mx-auto rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden cursor-pointer group transition-all hover:border-blue-500/50 shadow-lg">
                    {teacherForm.photo ? <img src={teacherForm.photo} className="w-full h-full object-cover" alt="Profile" /> : <div className="text-center space-y-1"><ImageIcon className="w-8 h-8 text-slate-800 mx-auto" /><p className="text-[6px] text-slate-600 font-black uppercase tracking-widest">Upload Foto</p></div>}
                    <div className="absolute inset-0 bg-blue-600/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Upload className="w-5 h-5 text-slate-800" /></div>
                ...
                <div className="flex flex-col gap-3 p-3 bg-slate-50/30 rounded-2xl border border-slate-200">
                  <div className="space-y-1.5">
                    <label className="label-luxe text-[6px] text-emerald-400 text-center block">Logo Pemda / Yayasan</label>
                    <div onClick={() => logoGovInputRef.current?.click()} className="relative aspect-[3/2] w-full rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden cursor-pointer group hover:border-emerald-500/50 transition-all shadow-inner">
                      {teacherForm.logoGov ? <img src={teacherForm.logoGov} className="w-full h-full object-contain p-2" /> : <Building2 className="w-5 h-5 text-slate-800" />}
                      <div className="absolute inset-0 bg-emerald-600/10 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all"><Upload className="w-4 h-4 text-slate-800" /></div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div onClick={() => logoSchoolInputRef.current?.click()} className="relative aspect-[3/2] w-full rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden cursor-pointer group hover:border-indigo-500/50 transition-all shadow-inner">
                      {teacherForm.logoSchool ? <img src={teacherForm.logoSchool} className="w-full h-full object-contain p-2" /> : <School className="w-5 h-5 text-slate-800" />}
                      <div className="absolute inset-0 bg-indigo-600/10 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all"><Upload className="w-4 h-4 text-slate-800" /></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5 text-left">
                    <div className="flex items-center gap-1.5">
                      <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Nama Lengkap</label>
                      {!isUnlocked && <Lock className="w-2.5 h-2.5 text-blue-500/50" />}
                    </div>
                    <div className="relative group" onClick={handleProtectedClick}>
                      <input 
                        readOnly={!isUnlocked}
                        value={teacherForm.name} 
                        onChange={e => isUnlocked && setTeacherForm({...teacherForm, name: e.target.value})} 
                        className={`w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none transition-all ${!isUnlocked ? 'cursor-pointer group-hover:border-blue-500/30' : 'focus:border-blue-500'} font-arial`} 
                      />
                      {!isUnlocked && <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-700"><Unlock className="w-3 h-3" /></div>}
                    </div>
                  </div>

                  <div className="space-y-1.5 text-left">
                    <div className="flex items-center gap-1.5">
                      <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Password Akses</label>
                    </div>
                    <button 
                      type="button"
                      ...
                      className="w-full bg-white hover:bg-slate-100 border border-slate-200 rounded-xl p-2.5 text-xs font-bold text-slate-800 transition-all flex items-center justify-between"
                    >
                      <span>Ganti Password Akses</span>
                      <ChevronRight className="w-4 h-4 text-slate-500" />
                    </button>
                    <p className="text-[7px] text-slate-500 italic">* Klik tombol di atas untuk mengganti password akses.</p>
                  </div>

                  <div className="space-y-1.5 text-left">
                    <div className="flex items-center gap-1.5">
                      <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Instansi / Sekolah</label>
                      {!isSchoolUnlocked && <Lock className="w-2.5 h-2.5 text-blue-500/50" />}
                    </div>
                    <div className="relative group" onClick={() => !isSchoolUnlocked && setShowSchoolPassModal(true)}>
                      <input 
                        ...
                        onChange={e => isSchoolUnlocked && setTeacherForm({...teacherForm, school: e.target.value})} 
                        className={`w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none transition-all ${!isSchoolUnlocked ? 'cursor-pointer group-hover:border-blue-500/30' : 'focus:border-blue-500'}`} 
                      />
                      {!isSchoolUnlocked && <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-700"><Unlock className="w-3 h-3" /></div>}
                    </div>
                  </div>

                  <div className="space-y-1.5 text-left">
                    <div className="flex items-center gap-1.5">
                      <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Password Instansi</label>
                    </div>
                    <button 
                      type="button"
                    ...
                    <p className="text-[7px] text-slate-500 italic">* Klik tombol di atas untuk mengganti password instansi.</p>
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">NIP / ID Pegawai</label>
                    <input value={teacherForm.nip} onChange={e => setTeacherForm({...teacherForm, nip: e.target.value})} className="w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none focus:border-blue-500/50 transition-all" />
                  </div>

                  <div className="space-y-1.5 text-left">
                    <div className="flex items-center gap-1.5">
                      <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Nomor WA/HP Guru BK</label>
                    </div>
                    <input 
                      placeholder="Contoh: 081234567890"
                      value={teacherForm.phone || ''} 
                      onChange={e => setTeacherForm({...teacherForm, phone: e.target.value})} 
                      className="w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none focus:border-blue-500 transition-all" 
                    />
                    <p className="text-[7px] text-slate-500 italic">* Digunakan untuk menerima laporan dari Kotak Masalah Siswa.</p>
                  </div>
                  
                  <div className="space-y-1.5">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Tahun Ajaran Aktif</label>
                    <input value={teacherForm.academicYear} onChange={e => setTeacherForm({...teacherForm, academicYear: e.target.value})} className="w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none focus:border-blue-500/50 transition-all" />
                  </div>

                  <div className="md:col-span-2 space-y-1.5">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Alamat Sekolah</label>
                    <textarea value={teacherForm.schoolAddress} onChange={e => setTeacherForm({...teacherForm, schoolAddress: e.target.value})} className="w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none h-16 focus:border-blue-500/50 transition-all" />
                  </div>

                  <div className="md:col-span-2 space-y-1.5">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Link Google Form (Integrasi Data)</label>
                    <input 
                      type="url"
                      value={teacherForm.googleFormUrl || ''} 
                      onChange={e => setTeacherForm({...teacherForm, googleFormUrl: e.target.value})} 
                      className="w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none focus:border-blue-500/50 transition-all" 
                      placeholder="https://docs.google.com/forms/d/e/.../viewform"
                    />
                    <p className="text-[7px] text-slate-500 italic">* Masukkan URL Google Form untuk mengintegrasikan pengisian data siswa.</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="relative">
              <button onClick={handleSaveTeacherData} className="w-full bg-blue-600 hover:bg-blue-500 py-3 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl transition-all text-white text-[9px]">Simpan Seluruh Perubahan</button>
              {showSaveNotification && (
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-emerald-500/90 text-white text-[8px] font-bold px-3 py-1 rounded-full shadow-lg flex items-center gap-1.5 animate-fade-in whitespace-nowrap">
                  <CheckCircle className="w-3 h-3" /> Data anda sudah tersimpan
                </div>
              )}
            </div>
          </div>
          ...
          <div className="glass-card p-5 rounded-[1.5rem] border border-indigo-500/10 space-y-5 shadow-xl animate-fade-in relative overflow-hidden bg-indigo-950/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input value={teacherForm.principalName || ''} onChange={e => setTeacherForm({...teacherForm, principalName: e.target.value})} className="w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none focus:border-indigo-500/50 font-arial" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest">NIP Kepala Sekolah</label>
                <input value={teacherForm.principalNip || ''} onChange={e => setTeacherForm({...teacherForm, principalNip: e.target.value})} className="w-full input-cyber rounded-xl p-2.5 text-xs font-bold outline-none focus:border-indigo-500/50" />
              </div>
            </div>
