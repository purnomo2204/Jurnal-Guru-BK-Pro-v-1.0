...
export interface AcademicEvent {
  ...
  title: string;
  type: 'pembelajaran' | 'libur' | 'lainnya' | 'mgbk' | 'ujian' | 'libur_sekolah' | 'dinas_dalam' | 'dinas_luar' | 'pelatihan';
  description?: string;
  reminder?: boolean;
}
...
export interface DailyJournal {
  ...
  day: string;
  time: string;
  activityType: 'Tugas Pokok' | 'Tugas Tambahan' | 'Lain - Lain';
  activityName: string;
  place: string;
  description: string;
  notes: string;
...
export interface StoredDocument {
  ...
  uploadDate: string;
  size: number;
  category: 'PROGRAM BK' | 'RPLBK' | 'LKPD' | 'MATERI' | 'TDA' | 'TES NON TES' | 'SERTIFIKAT' | 'SK' | 'DOKUMEN KEPEGAWAIAN' | 'LAINNYA';
}
