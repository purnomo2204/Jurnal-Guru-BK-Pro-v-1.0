...
import { Loader2, CheckCircle2, AlertCircle, X, ChevronLeft, Search, Users, BookOpen, Link, Wifi, WifiOff } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (path === '/dashboard') {
      if (![ViewMode.HOME, ViewMode.STUDENT_LIST, ViewMode.COLLABORATION, ViewMode.STUDENT_PERSONAL_BOOK, ViewMode.WORK_MECHANISM, ViewMode.STUDENT_AKPD].includes(view)) {
        setViewState(ViewMode.HOME);
      }
    } else if (path === '/input') {
      if (![ViewMode.INPUT_DATA_CATEGORY, ViewMode.STUDENT_INPUT, ViewMode.COUNSELING_INPUT, ViewMode.ANECDOTAL_RECORD_INPUT, ViewMode.ACHIEVEMENT_MANAGEMENT, ViewMode.STRATEGY_GENERATOR, ViewMode.VIOLATION_MANAGEMENT, ViewMode.ATTENDANCE_MANAGEMENT, ViewMode.COUNSELING_SCHEDULE, ViewMode.NEED_ASSESSMENT, ViewMode.HOME_VISIT, ViewMode.HOME_VISIT_INPUT, ViewMode.REFERRAL_MANAGEMENT, ViewMode.STUDENT_REPORT_INPUT, ViewMode.PARENT_COMMUNICATION, ViewMode.TDA_INPUT, ViewMode.REPORT_MUTATION, ViewMode.SOCIOMETRY].includes(view)) {
        setViewState(ViewMode.INPUT_DATA_CATEGORY);
      }
    } else if (path === '/laporan') {
      if (![ViewMode.REPORT_CATEGORY, ViewMode.COUNSELING_DATA, ViewMode.ANECDOTAL_RECORD_DATA, ViewMode.LPJ_MANAGEMENT, ViewMode.STRATEGY_HUB, ViewMode.STRATEGY_REPORTS, ViewMode.COMPONENT_RECAP, ViewMode.ANALYTICS, ViewMode.AUTOMATED_REPORTS, ViewMode.VULNERABILITY_MAP, ViewMode.STUDENT_DATA_REPORT].includes(view)) {
        setViewState(ViewMode.REPORT_CATEGORY);
      }
    } else if (path === '/jurnal') {
      if (view !== ViewMode.DAILY_JOURNAL_DATA) {
        setViewState(ViewMode.DAILY_JOURNAL_DATA);
      }
    } else if (path === '/jurnal/input') {
      if (view !== ViewMode.DAILY_JOURNAL_INPUT) {
        setViewState(ViewMode.DAILY_JOURNAL_INPUT);
      }
    } else if (path === '/kotak-masalah') {
      if (view !== ViewMode.PROBLEM_BOX) {
        setViewState(ViewMode.PROBLEM_BOX);
      }
    } else if (path === '/pendekatan-konseling') {
      if (view !== ViewMode.COUNSELING_APPROACHES) {
        setViewState(ViewMode.COUNSELING_APPROACHES);
      }
    } else if (path === '/teknik-konseling') {
      if (view !== ViewMode.COUNSELING_TECHNIQUES) {
        setViewState(ViewMode.COUNSELING_TECHNIQUES);
      }
    } else if (path === '/kamus-bk') {
      if (view !== ViewMode.COUNSELING_DICTIONARY) {
        setViewState(ViewMode.COUNSELING_DICTIONARY);
      }
    } else if (path === '/ice-breaking') {
      if (view !== ViewMode.ICE_BREAKING) {
        setViewState(ViewMode.ICE_BREAKING);
      }
    } else if (path === '/sop-bk') {
      if (view !== ViewMode.SOP_MANAGEMENT) {
        setViewState(ViewMode.SOP_MANAGEMENT);
      }
    } else if (path === '/kode-etik') {
      if (view !== ViewMode.KODE_ETIK) {
        setViewState(ViewMode.KODE_ETIK);
      }
    } else if (path === '/kolaborasi') {
      if (view !== ViewMode.COLLABORATION) {
        setViewState(ViewMode.COLLABORATION);
      }
    } else if (path === '/dokumen') {
      if (view !== ViewMode.DOCUMENT_MANAGEMENT) {
        setViewState(ViewMode.DOCUMENT_MANAGEMENT);
      }
    } else if (path === '/lkpd-materi') {
      if (view !== ViewMode.LKPD_MATERI_GENERATOR) {
        setViewState(ViewMode.LKPD_MATERI_GENERATOR);
      }
    } else if (path === '/papan-bimbingan') {
      if (view !== ViewMode.GUIDANCE_BOARD) {
        setViewState(ViewMode.GUIDANCE_BOARD);
      }
    } else if (path === '/settings-menu') {
      if (view !== ViewMode.SETTINGS_CATEGORY && view !== ViewMode.SETTINGS) {
        setViewState(ViewMode.SETTINGS_CATEGORY);
      }
    } else if (path === '/settings') {
      if (view !== ViewMode.SETTINGS) {
        setViewState(ViewMode.SETTINGS);
      }
    }
  ...
  const setView = (v: ViewMode) => {
    ...
    switch(v) {
      ...
      case ViewMode.STUDENT_INPUT: navigate('/input'); break;
      case ViewMode.COUNSELING_INPUT: navigate('/input'); break;
      case ViewMode.COUNSELING_DATA: navigate('/laporan'); break;
      case ViewMode.ANECDOTAL_RECORD_INPUT: navigate('/input'); break;
      case ViewMode.ANECDOTAL_RECORD_DATA: navigate('/laporan'); break;
      case ViewMode.LPJ_MANAGEMENT: navigate('/laporan'); break;
      case ViewMode.STRATEGY_HUB: navigate('/laporan'); break;
      case ViewMode.STRATEGY_REPORTS: navigate('/laporan'); break;
      case ViewMode.SETTINGS: navigate('/settings'); break;
      case ViewMode.WORK_MECHANISM: navigate('/dashboard'); break;
      ...
      case ViewMode.STRATEGY_GENERATOR: navigate('/input'); break;
      ...
      case ViewMode.HOME_VISIT_INPUT: navigate('/input'); break;
      case ViewMode.REFERRAL_MANAGEMENT: navigate('/input'); break;
      case ViewMode.STUDENT_AKPD: navigate('/dashboard'); break;
      case ViewMode.COLLABORATION: navigate('/kolaborasi'); break;
      case ViewMode.SETTINGS_CATEGORY: navigate('/settings-menu'); break;
      case ViewMode.INPUT_DATA_CATEGORY: navigate('/input'); break;
      case ViewMode.REPORT_CATEGORY: navigate('/laporan'); break;
      case ViewMode.REPORT_MUTATION: navigate('/input'); break;
      case ViewMode.STUDENT_REPORT_INPUT: navigate('/input'); break;
  ...
  // 1. Get IDs of students who match the search criteria
  ...
  const filteredEventLogs = useMemo(() => {
    ...
    return selectedAcademicYear ? eventLogs.filter(e => studentIdsInYear.has(e.studentId)) : eventLogs;
  ...
  const filteredAchievements = useMemo(() => {
    ...
    return selectedAcademicYear ? achievements.filter(a => studentIdsInYear.has(a.studentId)) : achievements;
  ...
  const filteredScholarships = useMemo(() => {
    ...
    return selectedAcademicYear ? scholarships.filter(s => studentIdsInYear.has(s.studentId)) : scholarships;
  ...
  const filteredEconomicallyDisadvantagedStudents = useMemo(() => {
    ...
    return selectedAcademicYear ? economicallyDisadvantagedStudents.filter(s => studentIdsInYear.has(s.studentId)) : economicallyDisadvantagedStudents;
  ...
  const [appearance, setAppearance] = useState<AppearanceConfig>(() => getSaved('guru_bk_appearance', {
    theme: 'light',
    font: 'jakarta',
    primaryColor: '#2563eb',
    moduleOrder: ['Input Data', 'Laporan', 'PUSAT PENGATURAN']
  }));
      ...
      try {
        ...
        try {
          ...
          if (e.code === 'app/duplicate-app') {
             ...
             app = getApp();
          } else if (e.message?.includes('invalid-api-key') || e.code?.includes('invalid-api-key')) {
            showNotification("Kunci API Firebase tidak valid. Silakan periksa kembali di Pengaturan.", "error");
            return;
          } else {
            throw e;
          }
  ...
  const syncDataToCloud = async (target: 'students' | 'logs' | 'groups' | 'events' | 'lpj' | 'test' | 'achievements' | 'violations' | 'akpd' | 'schedules' | 'parent_communications' | 'homeroom_journals' | 'tda' | 'sociometry' | 'daily_journals' | 'problem_reports' | 'guidance_materials' | 'scholarships' | 'report_mutations', payload: any, customUrl?: string | React.MouseEvent) => {
  ...
  const handleDeleteStudent = (id: string) => {
    ...
    // Optional: sync delete to cloud if needed
  };
  ...
  const renderContent = () => {
    ...
    switch (view) {
      case ViewMode.WELCOME: 
        return <WelcomeScreen onEnter={() => setView(ViewMode.HOME)} teacherData={teacherData} />;
      case ViewMode.HOME: 
        return (
          <Dashboard 
            key="home"
      ...
      case ViewMode.STUDENT_INPUT: 
        return (
          <StudentManagement onOpenPersonalBook={handleViewStudentBook} 
            ...
            onDelete={id => setStudents(s => s.filter(x => x.id !== id))} 
      ...
      case ViewMode.COUNSELING_DATA: 
        return (
          <CounselingManagement 
            ...
            onDelete={id => setCounselingLogs(l => l.filter(x => x.id !== id))} 
      ...
      case ViewMode.DAILY_JOURNAL_INPUT:
        return <DailyJournalInput onAdd={handleAddDailyJournal} setView={setView} />;
      ...
      case ViewMode.ANECDOTAL_RECORD_DATA:
        return (
          <AnecdotalRecordManagement 
            ...
            onDelete={id => setEventLogs(ev => ev.filter(x => x.id !== id))}
            teacherData={teacherData}
          />
        );
      ...
      case ViewMode.DEVELOPER_INFO:
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center bg-white rounded-3xl shadow-xl border border-slate-100 max-w-2xl mx-auto mt-10">
            <div className="w-24 h-24 bg-blue-100 rounded-full flex items-center justify-center mb-6 shadow-inner">
              <Users className="w-12 h-12 text-blue-600" />
            </div>
            <h2 className="text-2xl font-black text-slate-800 mb-4 tracking-tight uppercase">Pengembang Aplikasi</h2>
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans text-slate-700 selection:bg-primary/10">
      ...
      {isOffline && (
        <div className="fixed top-0 left-0 w-full z-[10000] bg-rose-500 text-white px-4 py-1.5 flex items-center justify-center gap-2 animate-in slide-in-from-top-full duration-300 shadow-lg">
          <AlertCircle className="w-4 h-4 animate-pulse" />
          <span className="text-[10px] font-black uppercase tracking-[0.2em]">Mode Offline Aktif — Data Tersimpan Secara Lokal</span>
        </div>
      )}
      <Routes>
          <Route path="/welcome" element={<WelcomeScreen onEnter={() => { setView(ViewMode.HOME); window.location.hash = '/dashboard'; }} teacherData={teacherData} />} />
        ...
        <AIChat students={filteredStudents} logs={filteredCounselingLogs} teacherData={teacherData} />
